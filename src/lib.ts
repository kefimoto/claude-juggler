...
function readConfig(): Config {
  if (!existsSync(CONFIG_PATH)) return { warningThreshold: 95, autoswapThreshold: 99, autoswapStrategy: "lowest" };
...
interface StateFileData {
  [claudeDir: string]: StateFile;
}

function ensureConfigDir(): void {
  mkdirSync(CONFIG_DIR, { recursive: true });
  if (!existsSync(ACCOUNTS_FILE)) writeJSON(ACCOUNTS_FILE, {} as AccountsFile);
  if (!existsSync(STATE_FILE)) writeJSON(STATE_FILE, {} as StateFileData);
}
...
function getStateForCurrentDir(): StateFile {
  ensureConfigDir();
  const allState = readJSON<StateFileData>(STATE_FILE);
  return allState[globalClaudeDir] || { active: null };
}

function saveAccountsForCurrentDir(accounts: { [name: string]: AccountData }): void {
  ensureConfigDir();
  const allAccounts = readJSON<AccountsFile>(ACCOUNTS_FILE);
...
function saveStateForCurrentDir(state: StateFile): void {
  ensureConfigDir();
  const allState = readJSON<StateFileData>(STATE_FILE);
...
export function groundTruthAccount(): string | null {
  ...
  const liveFp = fp(readJSON<{ claudeAiOauth: OauthToken }>(getCredsPath()).claudeAiOauth);
  const accounts = getAccountsForCurrentDir();
  for (const name of Object.keys(accounts)) {
    if (fp(accounts[name].claudeAiOauth) === liveFp) return name;
  }
  return null;
}
...
export function activate(targetName: string, quiet = false): string {
  ...
  if (claimedName !== null) {
    ...
    if (realName !== claimedName) {
      throw new Error(
        `ABORT: state claims ${claimedName} is live, but the live token ` +
...
export function prevAccount(): string {
  const accounts = listAccounts();
  if (accounts.length === 0) throw new Error("No accounts saved yet. Run `add <name>` first.");
  const current = currentAccount();
  const idx = current ? accounts.indexOf(current) : -1;
...
export function lowestUsageAccount(): string {
  const rows = statusAll();
  if (rows.length === 0) throw new Error("No accounts saved yet. Run `add <name>` first.");
  let lowest = rows[0];
  for (const r of rows) {
    const rPct = r.pct ?? 100;
    const lowPct = lowest.pct ?? 100;
    if (rPct < lowPct) lowest = r;
  }
  return lowest.name;
}
...
export function getAllInstalls(): string[] {
  ensureConfigDir();
...
export function listAccountsForInstall(claudeDir: string): string[] {
  ensureConfigDir();
  if (!existsSync(ACCOUNTS_FILE)) return [];
  const allAccounts = readJSON<AccountsFile>(ACCOUNTS_FILE);
...
export function getCurrentAccountForInstall(claudeDir: string): string | null {
  ensureConfigDir();
...
export function getAccountsForInstall(claudeDir: string): { [name: string]: AccountData } {
  ensureConfigDir();
  if (!existsSync(ACCOUNTS_FILE)) return {};
  const allAccounts = readJSON<AccountsFile>(ACCOUNTS_FILE);
...
export function install(options: InstallOptions | boolean = {}): void {
  // Support old boolean signature for backwards compatibility
  const opts: InstallOptions = typeof options === "boolean" ? { installCron: options } : options;
  ...
  let installMethod: "global" | "bunx" | "npx" = "global";
  ...
  while (!existsSync(join(pkgRoot, "package.json"))) {
    ...
    if (parent === pkgRoot) throw new Error("Could not find package root");
    pkgRoot = parent;
  }
  const hookScript = join(pkgRoot, "claude/hooks/check-usage-hook.sh");
  if (!existsSync(hookScript)) throw new Error(`Hook script not found at ${hookScript}`);
...
`;
  ...
  if (!settings.hooks.UserPromptSubmit) settings.hooks.UserPromptSubmit = [{ hooks: [] }];
  ...
  if (opts.noCron) {
    ...
    try {
      ...
      console.warn(`Warning: Could not setup cron job: ${(e as any).message}`);
    }
  }
...
function promptYesNo(question: string, defaultYes: boolean = false): boolean {
  const answer = prompt(question + (defaultYes ? " [Y/n] " : " [y/N] "));
  if (!answer) return defaultYes;
  return answer.toLowerCase() === "y" || answer.toLowerCase() === "yes";
}

function setupCronForInstall(command: string, claudeDir: string): void {
  try {
    // Get current crontab
    let crontab = "";
    try {
    ...
    throw new Error(`Failed to setup cron: ${(e as any).message}`);
  }
}

function removeCronForInstall(claudeDir: string): void {
  try {
    // Get current crontab
    let crontab = "";
    try {
